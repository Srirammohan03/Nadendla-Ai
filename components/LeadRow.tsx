...
const LeadRow: React.FC<LeadRowProps> = ({
  ...
  // Status check
  const isResearched =
    lead.status === "researched" || lead.status === "contacted";
  ...
  return (
    ...
    >
      ...
      {isResearched && (
        <div className="absolute left-0 top-0 bottom-0 w-1.5 bg-gray-400 rounded-l-lg"></div>
      )}
      {isNew && !isResearched && (
        ...
        <div className="flex items-center gap-2 mb-1">
          <h3
            className={clsx(
              ...
              isResearched ? "text-gray-600" : "text-gray-900",
            )}
          >
      ...
      <div className="w-24 shrink-0 text-sm hidden lg:block opacity-90">
        ...
        <div className="font-medium text-gray-700">
          {lead.date ? format(new Date(lead.date), "yyyy-MM-dd") : "—"}
        </div>
      </div>
      ...
      <div className="w-32 shrink-0 text-sm hidden xl:block opacity-90">
        ...
        <div
          className="font-medium text-gray-700 truncate text-xs"
          title={lead.portalId}
        >
          {lead.portalId}
        </div>
